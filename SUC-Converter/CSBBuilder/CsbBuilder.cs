...
namespace CsbBuilder.Builder
{
    public static class CsbBuilder
    {
        public static void Build(CsbProject project, string outputFileName)
        {
            ...
            CriTableSerializer.Serialize(outputFileName, cueSheetTables, CriTableWriterSettings.AdxSettings, 4096);

            if (cpkArchive.Count > 0)
            {
                cpkArchive.Save(Path.ChangeExtension(outputFileName, "cpk"), 4096);
            }
